...
use polymarket_client_sdk_v2::clob::types::{Amount, Side, OrderType, SignatureType};
...
pub struct PolymarketApi {
    ...
    clob_client: Arc<Mutex<Option<AuthenticatedClob>>>,
}
...
impl PolymarketApi {
    pub fn new(
        ...
        Self {
            ...
            clob_client: Arc::new(Mutex::new(None)),
        }
    }

    fn local_signer(&self) -> Result<LocalSigner<alloy::signers::k256::ecdsa::SigningKey>> {
        let private_key = self.private_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "Private key is required. Please set private_key in config.json"
            )
        })?;
    ...
    async fn authenticate_clob(
        ...
        if let Some(proxy_addr) = &self.proxy_wallet_address {
            ...
            let sig_type = resolve_proxy_signature_type(
                eoa,
                funder_address,
                derived_proxy,
                derived_safe,
            ...
            log::info!(
                "CLOB wallet: EOA={} funder={} derived_safe={:?} derived_proxy={:?} signature_type={:?}",
                eoa,
                funder_address,
                derived_safe,
                derived_proxy,
    ...
    async fn cached_clob_client(&self) -> Result<AuthenticatedClob> {
        self.clob_client
            .lock()
            .await
            .clone()
            .ok_or_else(|| {
                anyhow::anyhow!("Not authenticated with CLOB API. Call authenticate() first.")
            })
    }
    ...
    pub async fn authenticate(&self) -> Result<()> {
        ...
        let client = self.authenticate_clob(&signer).await?;

        if let Some(proxy_addr) = &self.proxy_wallet_address {
            eprintln!("Using proxy wallet: {}", proxy_addr);
        }

        *self.clob_client.lock().await = Some(client);
    ...
    /// Returns winning token id when market is closed and resolved, else None.
    pub async fn get_market_resolution(&self, condition_id: &str) -> Result<Option<String>> {
        ...
        for token in &market.tokens {
            if token.winner {
                return Ok(Some(token.token_id.clone()));
            }
        }
        Ok(None)
    }
    ...
    pub async fn get_best_price(&self, token_id: &str) -> Result<Option<TokenPrice>> {
        ...
        // Do not use bids[0]/asks[0]: REST/WS books are often sorted bids ascending / asks
        // descending, so first() is the *worst* price (e.g. ask 0.99). Use extrema instead.
        // Ref: https://github.com/Polymarket/rs-clob-client/issues/330
        ...
        // Empty or synthetic book: bid ~0.01, ask ~0.99
        if let Some(bid) = best_bid {
            if bid < Decimal::new(5, 2) && ask > Decimal::new(95, 2) {
                return Ok(None);
            }
        }
    ...
    /// Aggregated position size per outcome token id (`asset`) for a wallet (data-api).
    pub async fn get_position_sizes_for_assets(
        &self,
        wallet: &str,
        assets: &[String],
        ...
        let want: HashSet<&str> = assets.iter().map(|s| s.as_str()).collect();
        ...
        if !response.status().is_success() {
            anyhow::bail!("Positions API returned {}", response.status());
        }
        ...
        for p in rows {
            let asset = p
                .get("asset")
                .and_then(|v| v.as_str())
                .or_else(|| p.get("token").and_then(|v| v.as_str()));
            let Some(asset) = asset else { continue };
            if !want.contains(asset) {
                continue;
            }
            let size = p
                .get("size")
                ...
                .unwrap_or(0.0);
            *out.entry(asset.to_string()).or_insert(0.0) += size;
        }
        Ok(out)
    }
    ...
    pub async fn place_order(&self, order: &OrderRequest) -> Result<OrderResponse> {
        let signer = self.local_signer()?;
        let client = self.cached_clob_client().await?;
        ...
        let signed_order = client
            .sign(&signer, order_builder.build().await?)
            .await
        ...
        log::info!("✅ Order placed — order_id={}", response.order_id);
    ...
    pub async fn place_market_order(
        ...
        log::info!(
            "📤 Posting MARKET order: {} {} shares {} (type: {:?})",
            side,
            amount_decimal,
            token_id,
            order_type_enum
...
fn resolve_proxy_signature_type(
    eoa: AlloyAddress,
    funder: AlloyAddress,
    derived_proxy: Option<AlloyAddress>,
    derived_safe: Option<AlloyAddress>,
    configured: Option<u8>,
) -> Result<SignatureType> {
    if derived_safe == Some(funder) {
        if configured == Some(1) {
            warn!(
                "signature_type=1 (POLY_PROXY) but funder matches Gnosis Safe — using signature_type=2"
            );
        }
        return Ok(SignatureType::GnosisSafe);
    }
    if derived_proxy == Some(funder) {
        return Ok(SignatureType::Proxy);
    }
    if funder == eoa {
    ...
    match configured {
        Some(0) => Ok(SignatureType::Eoa),
        Some(1) => Ok(SignatureType::Proxy),
        Some(2) => Ok(SignatureType::GnosisSafe),
        None => {
            warn!(
                "Funder {funder} does not match derived Safe ({derived_safe:?}) or Proxy ({derived_proxy:?}); defaulting to GnosisSafe"
            );
            Ok(SignatureType::GnosisSafe)
        }
        Some(n) => anyhow::bail!(
            "Invalid signature_type: {n}. Must be 0 (EOA), 1 (Proxy), or 2 (GnosisSafe)"
        ),
    }
}
